...
import numpy as np
from envNEW import Env_cellular as env
...
class DDPG(object):
    ...
    def _build_a(self, s, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Actor', reuse=tf.AUTO_REUSE, custom_getter=custom_getter):
            # 确保输入形状正确
            s = tf.reshape(s, [-1, self.s_dim])  # 确保 s 的形状是明确的
    ...
    def _build_c(self, s, a, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Critic', reuse=tf.AUTO_REUSE, custom_getter=custom_getter):
            s = tf.reshape(s, [-1, self.s_dim])  # 确保 s 的形状是明确的
            a = tf.reshape(a, [-1, self.a_dim])  # 确保 a 的形状是明确的
            n_l1 = 64
...
for k in range(len(K_range)):
    ...
    for mct in range(ct):
        ...
        for i in range(MAX_EPISODES):
            ...
            ratek_DDPG = sum(reward_ddpg_vector) / MAX_EP_STEPS
...
plt.plot(K_range, rate_DDPG, "^-", label='DDPG: rewards')
plt.plot(K_range, rate_greedy, "+:", label='Greedy: rewards')
plt.plot(K_range, rate_random, "o--", label='Random: rewards')
...
''' Save final results'''
np.savez_compressed('data/results', rate_DDPG=rate_DDPG, rate_greedy=rate_greedy, rate_random=rate_random)
