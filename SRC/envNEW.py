...
class Env_cellular():
    def __init__(self,    MAX_EP_STEPS, s_dim, location_vector, location_GF,K, Pn,fading_n, fading_0):
        ...
        self.s_dim = s_dim  # 修复状态维度与实际状态匹配问题
    ...
    def reset(self):
        ...
        network_load = np.random.uniform(0.5, 1.5)  # 初始化网络负载
        state = self.channel_sequence[0, :].tolist()
        state.append(self.h0)
        state.append(batter_ini)
        state.append(network_load)  # 添加网络负载状态
        print(f"Debug - Reset State: {state}, Expected s_dim: {self.s_dim}")  # 调试输出
        return np.reshape(state, (1, self.s_dim))
